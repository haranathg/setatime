    ...
    <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-gray-950">
      <div className="max-w-3xl mx-auto px-4 py-5 space-y-4">
        <header className="text-center">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Principles</h2>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-relaxed max-w-lg mx-auto">
        ...
        <div className="flex justify-center">
          <div className="inline-flex bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden">
            <button
...
function TableMode({
  ...
  if (entries.length === 0 && !adding) {
    return (
      <div className="text-center space-y-4 py-6">
        <div className="text-[13px] text-gray-600 dark:text-gray-400 max-w-md mx-auto leading-relaxed">
  ...
  return (
    <>
      {/* Column headers */}
      <div className="grid grid-cols-3 gap-2 text-[10px] uppercase tracking-wider font-bold text-gray-500 dark:text-gray-400 px-3">
      ...
      {adding ? (
        ...
        <button
          onClick={onOpenAdd}
          className="w-full py-2 text-[12px] font-semibold text-gray-500 dark:text-gray-400 border border-dashed border-gray-300 dark:border-gray-700 rounded-xl hover:border-indigo-400 dark:hover:border-indigo-600 hover:text-indigo-700 dark:hover:text-indigo-300 dark:text-indigo-300"
        >
          + Add another
        </button>
      )}
...
function PrincipleRow({
  ...
  if (editing) {
    return (
      <div className="bg-white dark:bg-gray-900 border border-indigo-200 dark:border-indigo-800 rounded-xl p-3 space-y-2">
        <div className="grid grid-cols-3 gap-2">
          <textarea
            ...
            rows={3}
            className="w-full px-2 py-1.5 text-[12px] border border-gray-200 dark:border-gray-800 rounded focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none bg-white dark:bg-gray-900"
          />
          <textarea
            value={draft.value}
          ...
          <textarea
            ...
            rows={3}
            className="w-full px-2 py-1.5 text-[12px] border border-indigo-300 dark:border-indigo-700 rounded focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none bg-white dark:bg-gray-900"
          />
        </div>
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-3 group">
      <div className="grid grid-cols-3 gap-2 text-[13px] text-gray-800 dark:text-gray-200 leading-relaxed">
        <div className={entry.regret ? '' : 'text-gray-300 dark:text-gray-600 italic'}>
          {entry.regret || '—'}
        </div>
        <div className={entry.value ? '' : 'text-gray-300 dark:text-gray-600 italic'}>
          {entry.value || '—'}
        </div>
...
function CreedMode({ entries }: { entries: PrincipleEntry[] }) {
  if (entries.length === 0) {
    return (
      <div className="text-center text-[13px] text-gray-500 dark:text-gray-400 py-8">
  ...
  return (
    <div className="space-y-3 py-2">
      <div className="text-[10px] uppercase tracking-wider text-gray-400 dark:text-gray-500 text-center">
      ...
      <ul className="space-y-2">
        {entries.map((e, i) => (
          <li
            key={e.id}
            className="bg-white dark:bg-gray-900 border border-indigo-100 dark:border-indigo-900 rounded-2xl px-5 py-4 shadow-sm"
          >
            <div className="flex items-baseline gap-3">
              <span className="text-[10px] font-mono text-indigo-400 tabular-nums">
                {String(i + 1).padStart(2, '0')}
              </span>
              <p className="flex-1 text-base font-semibold text-indigo-900 dark:text-indigo-100 leading-relaxed">
                {e.action}
              </p>
            </div>
            {e.value && (
              <div className="text-[11px] text-gray-500 dark:text-gray-400 mt-1 pl-8">
